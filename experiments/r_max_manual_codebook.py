...
state_space_size = 2650 # Number of states in the state space
...
R_max = 4.0
R_max_min = 0.5  # Don't go to 0 — still need incentive for rare transitions
decay_rate = 0.05
m = 2
...
policy = np.random.choice([0,1,2,3],state_space_size) 
...
#Warmup phase
print("Warming up the R-Max model...")
for _ in range(50):   
    print("Warmup episode ", _)
    ...
    code = transform_to_code(img,tracker)
    ...
    while not done:
        ...
        next_code = transform_to_code(img_next,tracker)
...
load = False
if load == True:
    data = np.load("model_checkpoint.npz", allow_pickle=True)

    Q = data['Q']
    V = data['V']
    policy = data['policy']
    Q_prev = np.zeros_like(Q)  # Reset Q_prev to zeros
    Reward = data['Reward']
    R_max = data['R_max'].item()
    epsilon = data['epsilon'].item()
    episode = data['episode'].item()
    n_sa = data['n_sa']
    n_sa_true = data['n_sa_true']
    n_sas = data['n_sas']
    r_sa = data['r_sa']
    State_transition = data['State_transition']
    mapper = data['mapper'].item()


for episode in range(1000):


    if mapper.next_index > state_space_size - 50 and len(mapper.removed_indices) < 40:
    ...
    if episode % 5 == 0:
        ...
        plt.legend()
        plt.savefig('reward_per_episode_VQ_matrix10.png')
        plt.close()
